...
def main(argv):
  ...
  #  ipg:source, ipg:exitUrl, ipg:trackingId, ipg:channel, ipg:device, ipg:product
  ...
  if len(data) > 0:
    ...
    # TODO (0): empty order value / gmv calculations
    # TODO (0): cpc value calculations
    # TODO (0): order count
    ...
    data['ipg:product'] = data.apply(lambda x: parse.detect_product(x['ipg:exitUrl'], x['ipg:source']), axis=1)
